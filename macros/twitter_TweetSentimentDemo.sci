...
// Syntax
    ...
    while ~nlp_chk
        try
            ...
            [jars] = jar_import();
            if jars == "" // if user cancelled
                nlp_chk = %t 
            end
        end
    end
    
    // Twitter initalization
    if twitter_chk then
        if rhs == 0 then
            twitter = twitter_init()
            if isempty(twitter) then // if user cancelled
                demo_run = %f
            else
                demo_run = %t
            end
